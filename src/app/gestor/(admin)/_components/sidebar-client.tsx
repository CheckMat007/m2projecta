...
import { useTheme } from 'next-themes'; // 1. Importação do next-themes
...
const ThemeToggle = ({ isMobile = false }: { isMobile?: boolean }) => {
  ...
  if (!mounted) {
    return isMobile ? <div className="h-12 w-full mb-2" /> : <div className="w-10 h-10" />;
  }
  ...
  if (isMobile) {
    return (
      <button 
        ...
        className="w-full flex items-center gap-3 p-3 rounded-md text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900/50 hover:text-gray-900 dark:hover:text-white transition-colors mb-2"
      >
        {isDark ? <LucideIcons.Sun size={20} /> : <LucideIcons.Moon size={20} />}
        <span className="font-medium">Modo {isDark ? 'Claro' : 'Escuro'}</span>
      </button>
    );
  }
...
const DesktopUserDropdown = ({ user, isCollapsed }: { user: Session['user'], isCollapsed: boolean }) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className={`flex items-center gap-4 min-w-0 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left outline-none w-full ${isCollapsed ? 'justify-center rounded-full h-12 w-12' : 'p-2 rounded-md'}`}>
          <div className="w-12 h-12 rounded-full border-2 border-m2-green bg-gray-200 dark:bg-gray-700 flex items-center justify-center overflow-hidden flex-shrink-0">
            {user?.image ? <Image src={user.image} alt="Foto de perfil" width={48} height={48} className="object-cover w-full h-full" /> : <LucideIcons.User className="text-gray-500 dark:text-gray-400" />}
          </div>
          {!isCollapsed && (
            <>
...
const NavItem = ({ href, icon: Icon, label, isCollapsed, onClick }: { href: string; icon: ElementType; label: string; isCollapsed: boolean; onClick?: () => void }) => {
  ...
  const baseClasses = `flex items-center transition-colors ${isActive ? 'bg-m2-green text-black' : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'}`;
        ...
        if (item.subItems && isCollapsed) {
          return (
             <TooltipProvider key={item.label} delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={() => onItemClick && onItemClick()} className="flex items-center justify-center h-10 w-10 mx-auto rounded-md transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white">
        ...
        if (item.subItems && !isCollapsed) {
          return (
            <Collapsible key={item.label} open={openMenuLabel === item.label} onOpenChange={(isOpen) => setOpenMenuLabel(isOpen ? item.label : '')} className="space-y-1">
              <CollapsibleTrigger className="flex items-center justify-between w-full p-2 rounded-md transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white group">
                <div className="flex items-center gap-3"><Icon size={20} />{item.label}</div>
                <LucideIcons.ChevronDown size={16} className={`transition-transform duration-200 ${openMenuLabel === item.label ? 'rotate-180' : ''} text-gray-400 dark:text-gray-500 group-hover:text-gray-900 dark:group-hover:text-white`} />
              </CollapsibleTrigger>
              <CollapsibleContent className="pl-4 space-y-1 border-l border-gray-200 dark:border-gray-800 ml-4">
  ...
  return (
    <>
      {/* ========================================
        ...
        ========================================
      */}
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white/95 dark:bg-black/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4 z-50 transition-colors">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center text-gray-600 dark:text-gray-300">{children}</div>
          <Link href="/gestor/perfil" className="rounded-full w-10 h-10 border-2 border-m2-green overflow-hidden block">
             {user?.image ? (
                ...
                <LucideIcons.User className="w-full h-full p-2 bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400" />
             )}
          </Link>
        </div>
      </header>
      ...
      */}
      ...
      <aside className={`fixed inset-y-0 left-0 w-[85%] max-w-xs bg-gray-50 dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 z-50 transform transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 h-full flex flex-col">
          ...
          <Link href="/gestor/perfil" className="flex items-center gap-3 bg-white dark:bg-gray-900/50 p-3 rounded-2xl border border-gray-200 dark:border-gray-800 mb-6 hover:border-gray-300 dark:hover:border-gray-700 transition-all group shadow-sm dark:shadow-none">
            ...
            <div className="flex-1 min-w-0">
               <p className="font-bold text-gray-900 dark:text-white truncate text-base">{firstName}</p>
          ...
          <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-800">
             ...
             {/* 2. BOTÃO LIGHT/DARK NO MOBILE AQUI */}
             <ThemeToggle isMobile />

             <button 
                onClick={() => signOut({ callbackUrl: '/gestor/login' })} 
                className="w-full flex items-center gap-3 p-3 rounded-md text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-600 dark:hover:text-red-300 transition-colors mb-2"
             >
                <LogOut size={20} />
                <span className="font-medium">Sair da conta</span>
             </button>
             <div className="text-center text-xs text-gray-500 dark:text-gray-600 pb-2">Versão 2.1.0</div>
          </div>
        </div>
      </aside>
      ...
      {/* ========================================
        ...
        ========================================
      */}
      <aside className={`hidden md:flex fixed top-0 left-0 bg-white/95 dark:bg-black/95 backdrop-blur-sm h-screen flex-col border-r border-gray-200 dark:border-gray-800 transition-all duration-300 ease-in-out z-40 ${isCollapsed ? 'w-20 p-2' : 'w-64 p-4'}`}>
        ...
        <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-800 flex flex-col items-center w-full">
          ...
          {/* Grupo de Ações: Sino, TEMA e Botão Toggle */}
          <div className={`flex items-center justify-center ${isCollapsed ? 'flex-col gap-4 py-2' : 'flex-row gap-4'}`}>
             
             {/* Sino (Children) */}
             <div className="flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                {children}
             </div>

             {/* 3. BOTÃO LIGHT/DARK NO DESKTOP AQUI */}
             <ThemeToggle />

             {/* Botão de Recolher */}
             <Button variant="ghost" size="icon" onClick={toggleSidebar} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
          ...
          {!isCollapsed && (
             <div className="text-xs text-center text-gray-400 dark:text-gray-500 mt-4 pb-2">
               v2.1.0
             </div>
          )}
        </div>
