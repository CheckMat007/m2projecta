...
export function ProfileForm({ user }: { user: UserPrisma }) {
  ...
  // Handler for updating password (CORRECTED)
  async function handleUpdatePassword(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); // Prevent default form submission
    if (!passwordFormRef.current) return;

    const formData = new FormData(passwordFormRef.current); // Get data from the form
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <form ref={profileFormRef} onSubmit={handleUpdateProfile} className="space-y-6">
        ...
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center overflow-hidden">
            {file ? <Image src={URL.createObjectURL(file)} alt="Preview" width={96} height={96} className="object-cover w-full h-full" /> : user.image ? <Image src={user.image} alt="Foto de perfil" width={96} height={96} className="object-cover w-full h-full" /> : <User size={40} className="text-gray-500" />}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="space-y-2"><Label htmlFor="name">Nome</Label><Input id="name" name="name" defaultValue={user.name || ''} className="bg-gray-800 border-gray-700" /></div><div className="space-y-2"><Label htmlFor="email">E-mail</Label><Input id="email" name="email" type="email" defaultValue={user.email || ''} className="bg-gray-800 border-gray-700" /></div></div>
      ...
      {/* Change Password Form (CORRECTED) */}
